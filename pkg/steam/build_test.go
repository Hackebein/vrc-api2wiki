...
func TestExtractBuildFromBytes(t *testing.T) {
	...
	cb, err := ExtractBuildFromBytes(data, "public")
	if err != nil {
		t.Fatal(err)
	}
...
func TestExtractBuildMissing(t *testing.T) {
	_, err := ExtractBuildFromBytes([]byte("nothing here"), "public")
	if err == nil {
...
func TestClientBuildPages(t *testing.T) {
	cb, err := ExtractBuildFromBytes([]byte("2026.2.3p3-1867-42912f4b5c-Release"), "public")
	if err != nil {
		t.Fatal(err)
	}
